...
const STATES = ['waiting', 'delayed', 'active', 'completed', 'failed', 'cancelled'];
const TERMINAL_STATES = ['completed', 'failed', 'cancelled'];
const SCHEMA_VERSION = 1;
const DEFAULT_BACKOFF = { type: 'exponential', delayMs: 1_000, maxDelayMs: 60_000 };
const MAX_PAGE_SIZE = 500;
const MAX_PRUNE_BATCH_SIZE = 5_000;
const MAX_ERROR_MESSAGE_LENGTH = 8_192;
const MAX_ERROR_STACK_LENGTH = 32_768;
const ACTIVE_QUEUE_WORKSPACES = new Map();
const ACTIVE_QUEUE_DATABASES = new WeakMap();
class MinHeap {
    #values = [];
    #compare;
    constructor(compare) {
...
function jsonCloneWithSize(value, label, maxBytes = Number.MAX_SAFE_INTEGER) {
    let serialized;
    try {
...
function backoffDelay(backoff, attemptsMade) {
...
function serializeError(error) {
    ...
    try {
        ...
        message = 'Job handler threw a value that could not be converted to text';
    }
...
function publicJob(job) {
    ...
    return jsonClone(visible, 'job record');
}
/**
...
export class BroccoliQueue extends EventEmitter {
    ...
    #delayed = new Map();
    #leases = new Map();
    #terminal = new MinHeap((a, b) => a.finishedAt - b.finishedAt);
    #recentIds = [];
    #sequence = 0;
    #terminalCount = 0;
    #started = false;
    ...
    #closePromise;
    #closing = false;
    #namespaceRegistryKey;
    #usesDatabaseRegistry = false;
    #lastPersistenceFailureAt;
    #maintenancePromise;
    #maxJobDataBytes;
    #maxJobResultBytes;
    #maxBulkItems;
    ...
    #maintenanceMaxBatches;
    #dashboards = new Set();
    #dashboardStarts = new Set();
    constructor(options) {
        super();
        ...
        assert(this.defaultJobOptions && typeof this.defaultJobOptions === 'object' && !Array.isArray(this.defaultJobOptions), 'defaultJobOptions must be an object');
        ...
        if (this.#maxTerminalJobs !== undefined)
            safeInteger(this.#maxTerminalJobs, 'maxTerminalJobs', 0, 1_000_000);
    ...
    async #startInternal() {
        try {
            ...
            this.#jobs = this.db.getTable(`${this.namespace}_jobs`);
            this.#queues = this.db.getTable(`${this.namespace}_queues`);
            this.#metadata = this.db.getTable(`${this.namespace}_meta`);
            ...
            if (!metadata)
                this.#metadata.put('schema', { version: SCHEMA_VERSION, createdAt: Date.now() });
            this.#rebuildCounts();
    ...
    async addBulk(items) {
        ...
        for (const item of normalized) {
            ...
            if (duplicate) {
                result.push(publicJob(duplicate));
                continue;
            }
        ...
        for (const name of touched)
            this.#wakeWorkers(name);
        for (const job of created)
            this.#emitSafely('job', job);
        return result;
    }
    ...
    process(name, processor, options = {}) {
        ...
        const worker = new QueueWorker(this, name, processor, options);
    ...
    getJobs(query = {}) {
        this.#assertStarted();
        if (query.queueName !== undefined)
            assertQueueName(query.queueName);
        const limit = query.limit ?? 50;
        const offset = query.offset ?? 0;
        safeInteger(limit, 'limit', 1, MAX_PAGE_SIZE);
        safeInteger(offset, 'offset', 0);
        if (query.beforeId !== undefined)
            assert(typeof query.beforeId === 'string' && query.beforeId.length > 0, 'beforeId must be a non-empty job ID');
        ...
        if (query.beforeId !== undefined && cursorIndex < 0)
            return result;
        for (let index = cursorIndex - 1; index >= 0 && result.length < limit; index--) {
            const job = this.#jobs.get(this.#recentIds[index]);
            if (!job)
                continue;
            if (query.queueName !== undefined && job.queueName !== query.queueName)
                continue;
            if (states?.length && !states.includes(job.state))
                continue;
            if (matched++ < offset)
                continue;
    ...
    async retry(id, options = {}) {
        ...
        if (!current || current.state !== 'failed')
            return { retried: false, job: current ? publicJob(current) : null };
        const now = Date.now();
        const next = {
            ...current,
    ...
    async #cancel(id, eligibleStates) {
        this.#assertStarted();
        const current = this.#jobs.get(id);
        if (!current || !eligibleStates.includes(current.state))
            return false;
        const now = Date.now();
        const next = {
            ...current,
        ...
        this.#writeJob(next, current);
        for (const controller of this.#controllers.get(id) ?? [])
    ...
    async delete(id) {
        ...
        if (deleted) {
            ...
            if (job.state === 'waiting' || job.state === 'delayed')
                this.#compactQueueIndex(job.queueName);
            if (TERMINAL_STATES.includes(job.state))
                this.#compactTerminalIndex();
            await this.#flushWrites();
    ...
    async prune(options = {}) {
        ...
        while (removed < batchSize && inspected < maxInspected) {
            ...
            if (matches && current.finishedAt > cutoff
                && (this.#maxTerminalJobs === undefined || this.#terminalCount <= this.#maxTerminalJobs))
                break;
            inspected++;
            const entry = this.#terminal.pop();
            const job = this.#jobs.get(entry.id);
            if (!job || !TERMINAL_STATES.includes(job.state) || job.finishedAt !== entry.finishedAt)
                continue;
    ...
    async #closeInternal(options) {
        ...
        try {
            ...
            this.#closing = false;
            if (this.#started && !this.#maintenanceTimer)
    ...
    claimReady(name, workerId, limit, leaseDurationMs) {
        this.#assertStarted();
        ...
        while (claimed.length < limit && ready.size) {
            ...
            if (!latest || latest.sequence !== entry.sequence || !['waiting', 'delayed'].includes(latest.state) || latest.availableAt > now)
                continue;
    ...
    async recoverExpired(name, limit) {
        this.#assertStarted();
        assertQueueName(name);
        ...
        while (recovered < limit && inspected < maxInspected && (leases.peek()?.expiresAt ?? Infinity) <= now) {
            ...
            if (!latest || latest.state !== 'active' || latest.lockToken !== entry.token || latest.leaseExpiresAt !== entry.expiresAt)
                continue;
    ...
    async renewLease(id, token, leaseDurationMs) {
        safeInteger(leaseDurationMs, 'leaseDurationMs', 1, 24 * 60 * 60 * 1_000);
        const current = this.#jobs.get(id);
        const now = Date.now();
        if (!current || current.state !== 'active' || current.lockToken !== token || (current.leaseExpiresAt ?? 0) <= now)
            return false;
        this.#writeJob({ ...current, updatedAt: now, leaseExpiresAt: now + leaseDurationMs }, current);
        await this.#flushWrites();
        return true;
    }
    async completeClaim(id, token, result) {
        const current = this.#jobs.get(id);
        const now = Date.now();
        if (!current || current.state !== 'active' || current.lockToken !== token || (current.leaseExpiresAt ?? 0) <= now)
            return false;
        ...
        try {
            returnValue = result === undefined
                ? undefined
                : jsonCloneWithSize(result, 'job result', this.#maxJobResultBytes).value;
        }
        catch (error) {
    ...
    async failClaim(id, token, error) {
    ...
    async releaseClaims(claims, options = {}) {
        let released = false;
        for (const { id, token } of claims) {
            const current = this.#jobs.get(id);
            if (!current || current.state !== 'active' || current.lockToken !== token)
                continue;
            const now = Date.now();
            this.#writeJob({
                ...current,
    ...
    #ensureQueue(name, options = {}) {
        assert(options && typeof options === 'object' && !Array.isArray(options), 'queue options must be an object');
        const existing = this.#queues.get(name);
        if (existing)
            return existing;
        if (options.paused !== undefined)
            assert(typeof options.paused === 'boolean', 'paused must be a boolean');
        if (options.defaultJobOptions !== undefined) {
            assert(options.defaultJobOptions && typeof options.defaultJobOptions === 'object' && !Array.isArray(options.defaultJobOptions), 'defaultJobOptions must be an object');
    ...
    #normalizeOptions(name, options) {
        ...
        if (rawBackoff.maxDelayMs !== undefined)
            safeInteger(rawBackoff.maxDelayMs, 'backoff.maxDelayMs', 0, maximumDateDelay);
        if (values.idempotencyKey !== undefined) {
            assert(typeof values.idempotencyKey === 'string' && values.idempotencyKey.length > 0 && values.idempotencyKey.length <= 256, 'idempotencyKey must be a non-empty string no longer than 256 characters');
        }
        return {
            priority,
            delayMs,
    ...
    #writeJob(next, previous) {
        ...
        if (!previous) {
            ...
            if (this.#recentIds.length > 12_000)
                this.#recentIds.splice(0, 2_000);
        }
        else if (previous.state !== next.state) {
            this.#adjustCount(previous.queueName, previous.state, -1);
            this.#adjustCount(next.queueName, next.state, 1);
            if (TERMINAL_STATES.includes(previous.state))
                this.#terminalCount--;
            if (TERMINAL_STATES.includes(next.state))
                this.#terminalCount++;
        }
        if (next.state === 'waiting' || next.state === 'delayed')
            this.#pushPending(next);
        ...
        if (TERMINAL_STATES.includes(next.state) && next.finishedAt !== undefined && previous?.state !== next.state)
            this.#pushTerminal(next);
        if (previous && ['waiting', 'delayed'].includes(previous.state) && !['waiting', 'delayed'].includes(next.state))
            this.#compactQueueIndex(previous.queueName);
        if (previous && TERMINAL_STATES.includes(previous.state) && !TERMINAL_STATES.includes(next.state))
            this.#compactTerminalIndex();
    ...
    #adjustCount(queueName, state, amount) {
    ...
    #rebuildCounts() {
        this.#counts.clear();
        for (const queue of this.#queues.getAll())
            this.#counts.set(queue.name, emptyCounts());
        ...
        for (const job of jobs) {
            ...
            if (TERMINAL_STATES.includes(job.state))
                this.#pushTerminal(job);
        }
        const compareSequence = (a, b) => (a.sequence ?? a.createdAt) - (b.sequence ?? b.createdAt);
        const recent = new MinHeap(compareSequence);
        for (const job of jobs) {
            if (recent.size < 12_000)
                recent.push(job);
        ...
        while (recent.size)
            this.#recentIds.push(recent.pop().id);
    }
    #summary(record) {
        return {
            name: record.name,
    ...
    #pushPending(job) {
        ...
        if (job.state === 'waiting')
            this.#readyHeap(job.queueName).push(entry);
        else if (job.state === 'delayed')
            this.#delayedHeap(job.queueName).push({ ...entry, availableAt: job.availableAt });
    }
    #promoteDue(name, now) {
        ...
        while ((delayed.peek()?.availableAt ?? Infinity) <= now) {
            ...
            const job = this.#jobs.get(entry.id);
            if (!job || job.state !== 'delayed' || job.sequence !== entry.sequence || job.availableAt !== entry.availableAt)
                continue;
    ...
    #pushTerminal(job) {
        if (job.finishedAt !== undefined)
            this.#terminal.push({ id: job.id, finishedAt: job.finishedAt });
    }
    #compactLeaseIndex(name) {
        const activeCount = this.#counts.get(name)?.active ?? 0;
        const heap = this.#leaseHeap(name);
        if (heap.size <= activeCount * 2 + 1_000)
            return;
    ...
    #compactTerminalIndex() {
        if (this.#terminal.size <= this.#terminalCount * 2 + 1_000)
            return;
        const compacted = new MinHeap((a, b) => a.finishedAt - b.finishedAt);
        for (const state of TERMINAL_STATES) {
            for (const job of this.#jobs.query({ where: { state } })) {
                if (job.finishedAt !== undefined)
                    compacted.push({ id: job.id, finishedAt: job.finishedAt });
            }
        }
        this.#terminal = compacted;
    }
    #compactQueueIndex(name) {
        ...
        if (heapSize <= currentPending * 2 + 1_000)
            return;
        ...
        for (const state of ['waiting', 'delayed']) {
            for (const job of this.#jobs.query({ where: { queueName: name, state } })) {
                ...
                if (job.state === 'waiting')
                    ...
                    delayed.push({ ...entry, availableAt: job.availableAt });
            }
        }
        this.#ready.set(name, ready);
        this.#delayed.set(name, delayed);
    }
    #emitSafely(event, value) {
    ...
    #reportError(value) {
        ...
        try {
            ...
            error = new Error('Queue operation failed with an unprintable error');
        }
        if (this.listenerCount('error') > 0) {
            try {
                ...
                if (this.listenerCount('workerError') === 0)
                    return;
                try {
                    this.emit('workerError', listenerError instanceof Error ? listenerError : new Error(String(listenerError)));
                }
                catch { }
                return;
            }
    ...
    async #acquireNamespace() {
        ...
        const owners = ACTIVE_QUEUE_DATABASES.get(this.db) ?? new Map();
    ...
    #releaseNamespace() {
        if (this.#namespaceRegistryKey) {
            if (ACTIVE_QUEUE_WORKSPACES.get(this.#namespaceRegistryKey) === this)
                ACTIVE_QUEUE_WORKSPACES.delete(this.#namespaceRegistryKey);
            this.#namespaceRegistryKey = undefined;
        }
        if (this.#usesDatabaseRegistry) {
            const owners = ACTIVE_QUEUE_DATABASES.get(this.db);
            if (owners?.get(this.namespace) === this)
                owners.delete(this.namespace);
            this.#usesDatabaseRegistry = false;
        }
    }
    #scheduleMaintenance() {
        if (!this.#started || this.#closing || this.#maintenanceTimer)
            return;
        this.#maintenanceTimer = setInterval(() => { void this.#runMaintenance(); }, this.#maintenanceIntervalMs);
        this.#maintenanceTimer.unref?.();
    }
    async #runMaintenance() {
        ...
        const operation = (async () => {
            for (let batch = 0; batch < this.#maintenanceMaxBatches && !this.#closing; batch++) {
                ...
                if (removed < this.#maintenanceBatchSize)
                    break;
            }
        ...
        try {
            ...
            if (this.#maintenancePromise === operation)
    ...
    async #drainFlushWaiters() {
        if (this.#flushRunning || this.#flushWaiters.length === 0)
            return;
...
class QueueWorker {
    ...
    #timer;
    #active = new Map();
    constructor(queue, name, processor, options) {
        this.#queue = queue;
        this.queueName = name;
    ...
    async #stopInternal(drainTimeoutMs) {
        this.#running = false;
        if (this.#timer)
    ...
    async #pump() {
        if (!this.#running || this.#pumpPromise)
            return;
        this.#pumpPromise = (async () => {
            ...
            const claimed = this.#queue.claimReady(this.queueName, this.id, Math.min(this.#batchSize, available), this.#leaseDurationMs);
            if (claimed.length) {
                try {
                    await this.#queue.flushWrites();
                }
        ...
        try {
            ...
            if (this.#running)
                this.#schedule(this.#pollingIntervalMs);
        }
    }
    #run(claim) {
        ...
        const scheduleHeartbeat = () => {
            if (controller.signal.aborted || this.#active.get(job.id) !== active)
                return;
            active.heartbeat = setTimeout(() => {
                void this.#queue.renewLease(job.id, token, this.#leaseDurationMs).then(renewed => {
                    if (!renewed)
                        controller.abort(new Error('Job lease was taken by another worker'));
                }).catch(error => {
        ...
        active.task = Promise.resolve().then(async () => {
            ...
            try {
                ...
                try {
                    await this.#queue.failClaim(job.id, token, error);
                }
            ...
            if (this.#active.get(job.id) === active)
