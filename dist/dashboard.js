...
function parseAllowedOrigin(value) {
    ...
    try {
        ...
        throw new Error(`Invalid dashboard allowed origin: ${value}`);
    }
...
export async function createDashboardServer(queue, options = {}) {
    ...
    if (tls && (!tls.cert || !tls.key))
        throw new Error('dashboard TLS requires both a certificate and private key');
    ...
    if (!isLoopback(host) && !tls)
        throw new Error('dashboard TLS is required when binding outside loopback');
    ...
    const onRequest = async (request, response) => {
        try {
            ...
            if (!allowedHosts.has(requestHost))
                throw new HttpError(403, 'This dashboard host is not allowed');
            ...
            if (options.authToken) {
                ...
                if (!equalSecret(supplied, options.authToken))
                    throw new HttpError(401, 'Dashboard token required');
            }
            if (request.method !== 'GET') {
                const fetchSite = request.headers['sec-fetch-site']?.toLowerCase();
                if (fetchSite === 'cross-site')
                    throw new HttpError(403, 'Cross-site dashboard actions are not allowed');
                if (request.headers.origin !== undefined) {
                    ...
                    try {
                        ...
                        throw new HttpError(403, 'Cross-origin dashboard actions are not allowed');
                    }
            ...
            if (method === 'GET' && path === '/api/jobs') {
                ...
                const state = stateValue ? stateValue.split(',') : undefined;
            ...
            if (method === 'POST' && path === '/api/jobs') {
                ...
                if (typeof body.queueName !== 'string')
                    throw new HttpError(400, 'queueName is required');
                const job = await queue.add(body.queueName, body.data ?? null, (body.options ?? {}));
            ...
            if (method === 'DELETE' && jobMatch) {
                ...
                if (!removed)
                    throw new HttpError(409, 'Job cannot be deleted while active, or it no longer exists');
            ...
            if (!response.headersSent)
                sendJson(response, status, { error: messageFor(error, status) });
            else
                response.destroy();
        }
    };
